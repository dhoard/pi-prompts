...
error-handling problems, and API contract violations in the target
...
- Identify the module's documented contracts, public API semantics, and
  error guarantees.
- Understand the concurrency model, goroutine lifecycle management, and
  error-propagation patterns in use.
...
- Race conditions, resource leaks, and error-handling problems.
- Invalid state transitions, concurrency issues, and API contract violations.
- Nil dereference risks, boundary conditions, and test coverage gaps that
...
- Every source file, interface, and test in the target module has been
  inspected.
- Every finding is tied to a specific file and function or type.
