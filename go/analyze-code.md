...
Analyze the target package for correctness issues and potential bugs.
Produce findings only. Do not write or modify code.

## Objective

Identify runtime bugs, logic errors, race conditions, resource leaks,
error handling problems, API contract violations, and deterministic
behavior risks in the target package. Report only evidence-backed findings
...
guidance, or an issue reference for the target package.
...
- Read every relevant source file, interface, test file, and build or
  configuration file for the target package.
- Read repository guidance that defines coding, testing, validation, or agent
  behavior.
- Identify documented contracts, public API semantics, error contracts, and
  compatibility constraints.
- Understand the goroutine, channel, mutex, context, and lifecycle model where relevant.
...
- Exact file path and function, method, or type.
...
- The target package cannot be located.
