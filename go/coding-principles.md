...
- **Match the existing style.** If the file uses short variable declarations, use them. If it uses a particular naming convention, follow it. Consistency within a file beats your personal preference.
...
- **Don't write tests for the sake of writing tests.** A test that checks whether a constructor sets fields is worthless. Test behavior, not implementation.
...
- For coverage improvement guidance, see `go-code-coverage.md`.
...
- **Read the error message.** The whole thing, including the stack trace. A panic with a nil pointer dereference could mean a hundred different things. The message and trace tell you which one.
...
- **Don't add workarounds without understanding the root cause.** A nil check might prevent a panic, but the underlying bug is still there.
...
- **Write specific commit messages.** "Fix bug" is useless. "Fix nil pointer in user lookup when email contains uppercase chars" tells the next person exactly what happened.
