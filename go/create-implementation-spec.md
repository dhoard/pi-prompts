...
function signatures, behavior rules, error handling — concrete enough that
...
- The minimal test file and test function the implementer should add or
...
- The expected pre-fix failure mode: assertion failure, panic, error
...
List every test file and test function that will validate the work. Describe
the test scenarios: happy path, edge cases, error conditions.

For bugs and issues, the first test listed must be the minimal
reproduction test. Include the file path, test function name,
...
### Exact Files and Packages to Modify

Full paths to every source file that will be changed.

### New Types and Interfaces to Create

Fully qualified names with a brief description of each.

### Function Signatures

Exact signatures for every function: receiver, return type, name,
parameters.

### Behavior Rules

Detailed behavior for each changed or new function. Contractual obligations:
preconditions, postconditions, invariants.

### Error Handling

How invalid inputs, system errors, and edge cases are handled. Error types
and messages.
...
- Goroutine-safety guarantees, locking strategy, resource cleanup.
