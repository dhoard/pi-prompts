...
The `.pi/plans/` directory should be listed in the project's `.gitignore`
to prevent plan files from being committed. If the directory does not
exist, create it.
...
- Identify the relevant build or configuration files, if they constrain
  the design.
- Note any constraints that must be preserved: Go version compatibility,
  public API semantics, error contracts, and nil-safety contracts.
...
- Types, interfaces, and test files in the affected area.
...
- Files and types to modify or create.
- Key function signatures and data flow.
...
- Whether public API, function signatures, or error contracts change.
